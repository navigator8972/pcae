'''
GMM compatible with pytorch: extended from https://github.com/ldeecke/gmm-torch/blob/master/gmm.py
'''
import torch
import numpy as np

from math import pi

from sklearn.cluster import KMeans

class GaussianMixture(torch.nn.Module):
    """
    Fits a mixture of k=1,..,K Gaussians to the input data. Input tensors are expected to be flat with dimensions (n: number of samples, d: number of features).
    The model then extends them to (n, k: number of components, d).
    The model parametrization (mu, sigma) is stored as (1, k, d), and probabilities are shaped (n, k, 1) if they relate to an individual sample, or (1, k, 1) if they assign membership probabilities to one of the mixture components.
    
    ------------Line--------------
    The original code only supports diag covariance matrices, because of any difficulty of autodiff in matrix inverse? I guess there shouldn't be anything prevent us
    from doing that, well speed might be a concern but lets try to support more...
    Also, add an initialization based on K-Means as we always do...
    """
    def __init__(self, n_components, n_features, mu_init=None, var_init=None, var_type='diag', eps=1.e-6):
        """
        Initializes the model and brings all tensors into their required shape. The class expects data to be fed as a flat tensor in (n, d). The class owns:
            x:              torch.Tensor (n, k, d)
            mu:             torch.Tensor (1, k, d)
            var:            torch.Tensor (1, k, d)
            pi:             torch.Tensor (1, k, 1)
            eps:            float
            n_components:   int
            n_features:     int
            score:          float
        args:
            n_components:   int
            n_features:     int
            mu_init:        torch.Tensor (1, k, d)
            var_init:       torch.Tensor (1, k, d)
            eps:            float
        """

        super(GaussianMixture, self).__init__()

        self.eps = eps
        self.n_components = n_components
        self.n_features = n_features
        self.score = -np.inf

        self.var_type = var_type
        assert var_type in ['full', 'diag']

        if mu_init is not None:
            assert mu_init.size() == (1, n_components, n_features), "Input mu_init does not have required tensor dimensions (1, %i, %i)" % (n_components, n_features)
            # (1, k, d)
            self.mu = torch.nn.Parameter(mu_init, requires_grad=False)
        else:
            self.mu = torch.nn.Parameter(torch.randn(1, n_components, n_features), requires_grad=False)

        if var_init is not None:
            if var_type == 'full':
                assert var_init.size() == (1, n_components, n_features, n_features), "Input var_init does not have required tensor dimensions (1, %i, %i, %i)" % (n_components, n_features, n_features)
            else:
                assert var_init.size() == (1, n_components, n_features), "Input var_init does not have required tensor dimensions (1, %i, %i)" % (n_components, n_features)
            # (1, k, d)
            self.var = torch.nn.Parameter(var_init, requires_grad=False)
        else:
            if var_type == 'full':
                tmp_var = torch.eye(n_features)
                tmp_var = tmp_var.reshape((1, 1, n_features, n_features))
                self.var = torch.nn.Parameter(tmp_var.repeat(1, n_components, 1, 1), requires_grad=False)
            else:
                self.var = torch.nn.Parameter(torch.ones(1, n_components, n_features), requires_grad=False)

        # (1, k, 1)
        self.pi = torch.nn.Parameter(torch.Tensor(1, n_components, 1), requires_grad=False).fill_(1./n_components)

    def kmeans_init(self, x, n_init=1):
        '''
        initialize parameter with kmeans
        '''
        '''
        can only do this on cpu i guess? no idea how well scikit-cuda works...
        '''
        '''
        expecting x = torch.Tensor((n, d))
        '''
        if next(self.parameters()).is_cuda:
            is_cuda = True
        else:
            is_cuda = False

        if isinstance(x, np.ndarray):
            x_cpu = x
        else:
            if is_cuda:
                x_cpu = x.cpu().data.numpy()
            else:
                x_cpu = x.numpy()    
        
        kmeans_estimator = KMeans(init='k-means++', n_clusters=self.n_components, n_init=n_init)
        resp = np.zeros((x_cpu.shape[0], self.n_components, 1)).astype(np.float64)  #use double because of small likelihood of high-dim GMM
        labels = kmeans_estimator.fit(x).labels_
        resp[np.arange(x_cpu.shape[0]), labels, [0]*x_cpu.shape[0]] = 1.
        
        weights = torch.from_numpy(resp)

        if is_cuda:
            weights.cuda()

        if isinstance(x, np.ndarray):
            x_tensor = torch.from_numpy(x)
        else:
            x_tensor = x
        
        x_tensor = x_tensor.unsqueeze(1).expand(x_tensor.size(0), self.n_components, x_tensor.size(1))
        
        pi_init, mu_init, var_init = self.__m_step(x_tensor, weights)

        self.__update_pi(pi_init)
        self.__update_mu(mu_init)
        self.__update_var(var_init)

        return

    def fit(self, x, n_iter=1000, delta=1e-8, kmeans_init=False):
        """
        Public method that fits data to the model.
        args:
            n_iter:     int
            delta:      float
        """
        if kmeans_init:
            self.kmeans_init(x)
            
        if len(x.size()) == 2:
            # (n, d) --> (n, k, d)
            x = x.unsqueeze(1).expand(x.size(0), self.n_components, x.size(1))

        i = 0
        j = np.inf

        while (i <= n_iter) and (j >= delta):

            old_score = self.score
            old_mu = self.mu
            old_var = self.var

            self.__em(x)
            self.score = self.__score(self.pi, self.__p_k(x, self.mu, self.var))

            if (self.score.abs() == float("Inf")) or (self.score == float("nan")):
                # when the log-likelihood assumes inane values, reinitialize model
                self.__init__(self.n_components, self.n_features)

            i += 1
            j = self.score - old_score
            #print('score improvement:', j)
            if j <= delta:
                # when the score decreases, revert to old parameters
                self.__update_mu(old_mu)
                self.__update_var(old_var)


    def predict(self, x, probs=False):
        """
        Assigns input data to one of the mixture components by evaluating the likelihood under each. If probs=True returns normalized probabilities of class membership instead.
        args:
            x:          torch.Tensor (n, d) or (n, k, d)
            probs:      bool
        returns:
            y:          torch.LongTensor (n)
        """

        if len(x.size()) == 2:
            # (n, d) --> (n, k, d)
            x = x.unsqueeze(1).expand(x.size(0), self.n_components, x.size(1))

        p_k = self.__p_k(x, self.mu, self.var)
        if probs:
            return p_k / (p_k.sum(1, keepdim=True) + self.eps)
        else:
            _, predictions = torch.max(p_k, 1)
            return torch.squeeze(predictions).type(torch.LongTensor)
    
    def likelihood(self, x):
        if len(x.size()) == 2:
            # (n, d) --> (n, k, d)
            x = x.unsqueeze(1).expand(x.size(0), self.n_components, x.size(1))
        return torch.sum(self.pi * self.__p_k(x, self.mu, self.var), 1).squeeze()
    
    def sample(self, n_samples=1, data_type=np.float32):
        #use numpy rand here because we do not expect to backpropagate the sampling process
        if next(self.parameters()).is_cuda:
            is_cuda = True
        else:
            is_cuda = False

        selected_comps = np.random.randint(0, self.n_components, size=n_samples)

        def apply_diag(cov):
            return np.diag(cov) if self.var_type == 'diag' else cov
                
        if is_cuda:
            samples_numpy = [np.random.multivariate_normal(mean=(self.mu[0, c]).cpu().data.numpy(), cov=apply_diag(self.var[0, c].cpu().data.numpy())) for c in selected_comps]
        else:
            samples_numpy = [np.random.multivariate_normal(mean=(self.mu[0, c]).numpy(), cov=apply_diag(self.var[0, c, :].numpy())) for c in selected_comps]
        return torch.from_numpy(np.array(samples_numpy).astype(data_type))


    def __p_k(self, x, mu, var):
        """
        Returns a tensor with dimensions (n, k, 1) indicating the likelihood of data belonging to the k-th Gaussian.
        args:
            x:      torch.Tensor (n, k, d)
            mu:     torch.Tensor (1, k, d)
            var:    torch.Tensor (1, k, d)
        returns:
            p_k:    torch.Tensor (n, k, 1)
        """

        # (1, k, d) --> (n, k, d)
        mu = mu.expand(x.size(0), self.n_components, self.n_features)
        if self.var_type == 'diag':
            var = var.expand(x.size(0), self.n_components, self.n_features)
            # (n, k, d) --> (n, k, 1)
            exponent = torch.exp(-.5 * torch.sum((x - mu) * (x - mu) / var, 2, keepdim=True))
            # (n, k, d) --> (n, k, 1)
            prefactor = torch.rsqrt(((2. * pi) ** self.n_features) * torch.prod(var, dim=2, keepdim=True) + self.eps)
        elif self.var_type == 'full':
            chol = torch.cholesky(self.var, upper=False)     #(1, k, d, d)
            prefactor = torch.rsqrt(((2. * pi) ** self.n_features) * torch.prod(torch.diagonal(chol, dim1=-2, dim2=-1).pow(2), dim=2, keepdim=True) + self.eps)
            #chol = chol.expand(x.size(0), self.n_components, self.n_features, self.n_features)  #(n, k, d, d)
            # potrs is not differentiable as for now <2019-Mar-12th>, need to think about other way. really dont want to use inverse, ahhhhhhhhhh
            #c = torch.potrs((x-mu).unsqueeze(-1), chol, upper=False).squeeze(-1) #(n, k, d, 1) vs (n, k, d, d)
            #exponent = torch.exp(-.5 * torch.sum((x-mu)*c, dim=2, keepdim=True)) #(n, k, 1, d).dot((n, k, d, 1)) --> (n, k, 1)
            c = torch.matmul(torch.matmul((x-mu).unsqueeze(-2), torch.inverse(self.var)), (x-mu).unsqueeze(-1))  #((n, k, 1, d).dot((n, k, d, d))).dot(n, k, d, 1) --> (n, k, 1, 1)
            exponent = torch.exp(-.5 * c.squeeze(-1))
        else:
            raise NotImplementedError
               
        return prefactor * exponent


    def __e_step(self, pi, p_k):
        """
        Computes weights that indicate the probabilistic belief that a data point was generated by one of the k mixture components. This is the so-called expectation step of the EM-algorithm.
        args:
            pi:         torch.Tensor (1, k, 1)
            p_k:        torch.Tensor (n, k, 1)
        returns:
            weights:    torch.Tensor (n, k, 1)
        """

        weights = pi * p_k
        return torch.div(weights, torch.sum(weights, 1, keepdim=True) + self.eps)


    def __m_step(self, x, weights):
        """
        Updates the model's parameters. This is the maximization step of the EM-algorithm.
        args:
            x:          torch.Tensor (n, k, d)
            weights:    torch.Tensor (n, k, 1)
        returns:
            pi_new:     torch.Tensor (1, k, 1)
            mu_new:     torch.Tensor (1, k, d)
            var_new:    torch.Tensor (1, k, d)
        """

        # (n, k, 1) --> (1, k, 1)
        n_k = torch.sum(weights, 0, keepdim=True)
        pi_new = torch.div(n_k, torch.sum(n_k, 1, keepdim=True) + self.eps)
        # (n, k, d) --> (1, k, d)
        mu_new = torch.div(torch.sum(weights * x, 0, keepdim=True), n_k + self.eps)
        #print('pi:', self.pi, 'mu:', self.mu, 'var:', self.var)
        
        if self.var_type == 'diag':
            # (n, k, d) --> (1, k, d)
            var_new = torch.div(torch.sum(weights * (x - mu_new) * (x - mu_new), 0, keepdim=True), n_k + self.eps)
        elif self.var_type == 'full':
            # (n, k, d) --> (1, k, d, d)
            # think more about this...

            # mat_A = (weights * (x - mu_new)).reshape(-1, x.shape[-1]).unsqueeze(2)  # (n, k, d) --> (n*k, d) --> (n*k, d, 1)
            # mat_B = (x - mu_new).reshape(-1, x.shape[-1]).unsqueeze(1)              # (n, k, d) --> (n*k, d) --> (n*k, 1, d)
            # # (n*k, d, 1) and (n*k, 1, d) --> (n*k, d, d) --> (n, k, d, d)
            # batch_outprod = torch.bmm(mat_A, mat_B).reshape(x.shape[0], self.n_components, x.shape[-1], x.shape[-1])  
            batch_outprod = torch.matmul((weights * (x - mu_new)).unsqueeze(3), (x - mu_new).unsqueeze(2)) 
            var_new = torch.div(torch.sum(batch_outprod, 0, keepdim=True), n_k.unsqueeze(-1) + self.eps)
        return pi_new, mu_new, var_new


    def __em(self, x):
        """
        Performs one iteration of the expectation-maximization algorithm by calling the respective subroutines.
        args:
            x:          torch.Tensor (n, k, d)
        """

        weights = self.__e_step(self.pi, self.__p_k(x, self.mu, self.var))
        pi_new, mu_new, var_new = self.__m_step(x, weights)

        self.__update_pi(pi_new)
        self.__update_mu(mu_new)
        self.__update_var(var_new)


    def __score(self, pi, p_k):
        """
        Computes the log-likelihood of the data under the model.
        args:
            pi:         torch.Tensor (1, k, 1)
            p_k:        torch.Tensor (n, k, 1)
        """

        weights = pi * p_k
        return torch.sum(torch.log(torch.sum(weights, 1) + self.eps))


    def __update_mu(self, mu):
        """
        Updates mean to the provided value.
        args:
            mu:         torch.FloatTensor
        """

        assert mu.size() in [(self.n_components, self.n_features), (1, self.n_components, self.n_features)], "Input mu does not have required tensor dimensions (%i, %i) or (1, %i, %i)" % (self.n_components, self.n_features, self.n_components, self.n_features)

        if mu.size() == (self.n_components, self.n_features):
            self.mu = mu.unsqueeze(0)
        elif mu.size() == (1, self.n_components, self.n_features):
            self.mu.data = mu


    def __update_var(self, var):
        """
        Updates variance to the provided value.
        args:
            var:        torch.FloatTensor
        """
        if self.var_type == 'diag':
            assert var.size() in [(self.n_components, self.n_features), (1, self.n_components, self.n_features)], "Input var does not have required tensor dimensions (%i, %i) or (1, %i, %i)" % (self.n_components, self.n_features, self.n_components, self.n_features)
        elif self.var_type == 'full':
            assert var.size() in [(self.n_components, self.n_features, self.n_features), (1, self.n_components, self.n_features, self.n_features)], "Input var does not have required tensor dimensions (%i, %i) or (1, %i, %i)" % (self.n_components, self.n_features, self.n_features, self.n_components, self.n_features, self.n_features)
        if var.size() == (self.n_components, self.n_features):
            self.var = var.unsqueeze(0)
        elif var.size() == (1, self.n_components, self.n_features):
            self.var.data = var
        elif var.size() == (1, self.n_components, self.n_features, self.n_features):
            self.var.data = var


    def __update_pi(self, pi):
        """
        Updates pi to the provided value.
        args:
            pi:         torch.FloatTensor
        """

        assert pi.size() in [(1, self.n_components, 1)], "Input pi does not have required tensor dimensions (%i, %i, %i)" % (1, self.n_components, 1)

        self.pi.data = pi